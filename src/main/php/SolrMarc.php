...
class SolrMarc extends SolrDefault
{
   ...
   /**
    ...
    public function __construct(Config $mainConfig = null, Config $recordConfig = null,
        YamlReader $yamlReader, Config $searchSettings = null, string $solrMarcYaml = null
    ) {
     ...
     * @return void
     */
    public function addSolrMarcYaml(string $solrMarcYaml, bool $parse = true) : void
    {
        if (!in_array($solrMarcYaml, $this->solrMarcYamls, false)) {
     ...
     * @return mixed[]
     */
    private function getSolrMarcSpecs(string $item) : array
    {
     ...
     * @return mixed[]
     */
    public function getSolrMarcKeys(string $category = '', bool $others = false) : array
    {
        if (empty($this->solrMarcSpecs)) {
            foreach ($this->solrMarcYamls as $solrMarcYaml) {
                $this->parseSolrMarcSpecs($solrMarcYaml);
        ...
        if (empty($category)) {
            if (!$others) {
                if (($key = array_search('other', $specKeys, false)) !== false) {
                    unset($specKeys[$key]);
                }
            }
            return $specKeys;
     ...
     * @return void
     */
    private function parseSolrMarcSpecs(string $solrMarcYaml) : void
    {
     ...
     * @return mixed[]
     */
    public function getMarcData(string $dataName) : array
    {
                ...
                foreach ($this->getMarcRecord()->getFields($field) as $index => $fieldObject) {
                    ...
                    if (!empty($subFieldSpecs['subfields'])) {
                        ...
                        foreach ($subFieldList as $subfield => $properties) {
                            $fieldData = [];
                            if (strpos((string)$subfield, 'indicator') !== false) {
                                $indicator = substr((string)$subfield, 9, 1);
     ...
     * @return mixed[]
     */
    protected function getOriginalLetters() : array
    {
        $index = '';
        $originalLetters = [];
        if ($fields = $this->getMarcRecord()->getFields('880')) {
            foreach ($fields as $field) {
                ...
                foreach ($subfields as $subfield) {
                    ...
                    if ($code == '6') {
                        $index = (string)preg_replace('/-.*$/', '', $subfield->getData());
     ...
     * @param RecordLinker $recordLink Record link helper (optional; may be used to
    ...
    {
        if (array_key_exists('publishPlace', $this->fields)) {
    ...
    {
        if (array_key_exists('collection_details', $this->fields)) {
            return $this->fields['collection_details'];
        }
        return array();
    }
     ...
     * @return mixed[]
     */
    protected function getArticleOpenUrlParams() : array
    {
        ...
        if (isset($pagesData[0]['pages']['data'][0])) {
            ...
            if (strpos($pages, '-') !== false) {
                list(, $epage) = explode('-', $pagesData[0]['pages']['data'][0]);
